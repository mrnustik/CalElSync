...
{
    private readonly IDictionary<string, ICollection<TodoTask>> _tasks =
        new Dictionary<string, ICollection<TodoTask>>();
    ...
    public Task<IReadOnlyCollection<TodoTask>> GetTasksAsync(string projectId, CancellationToken ct)
    {
        ...
        return Task.FromResult<IReadOnlyCollection<TodoTask>>(projectTasks.ToArray().AsReadOnly());
    }
    ...
    public void AddTask(string projectId, TodoTask task)
    {
        ...
        {
            var newCollection = new List<TodoTask> { task };
